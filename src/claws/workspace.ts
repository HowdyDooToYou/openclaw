...
function ensureClawWorkspaceTables(db: DatabaseSync): void {
  db.exec(`
...
function diagnostic(entry: ClawApplyPlanEntry, code: string, message: string): ClawDiagnostic {
  return { level: "error", code, path: `$.entries[${entry.id}]`, message };
}
...
async function prepareWorkspaceFile(params: {
  ...
  const { entry, manifestRoot, workspaceRoot, clawId, existingRef, targetRefs } = params;
  if (!entry.source) {
    return diagnostic(
      entry,
  ...
  try {
    ...
    return diagnostic(entry, code, message);
  }
}
...
function readWorkspaceRefsForRoot(
  ...
  ensureClawWorkspaceTables(database.db);
  return database.db
    .prepare(
...
function workspaceDiagnosticFromError(entry: ClawApplyPlanEntry, error: unknown): ClawDiagnostic {
  const message = error instanceof Error ? error.message : String(error);
  const code =
    error instanceof FsSafeError ? `workspace_file_${error.code}` : "workspace_file_io_error";
  return diagnostic(entry, code, message);
...
export async function applyClawWorkspaceFiles(
  ...
  try {
    ...
    throw new ClawWorkspaceApplyError(
      entries.map((entry) => workspaceDiagnosticFromError(entry, error)),
    );
  }
  ...
  for (const item of prepared) {
    if (item.operation !== "unchanged") {
      try {
        ...
        const code =
          error instanceof FsSafeError ? `workspace_file_${error.code}` : "workspace_file_io_error";
        throw new ClawWorkspaceApplyError([diagnostic(item.entry, code, message)]);
      }
    }
